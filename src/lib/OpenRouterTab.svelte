<script>
	import { apiState, setApiKey, addMessage, clearMessageHistory, forceStop, messageList, currentMessage, enableThinking, getMessageDetails } from '$lib/openrouter.js';
	...
	async function handleDownload() {
		...
		link.download = 'WebVM_OpenRouter.png';
		link.click();
	}
...
<h1 class="text-lg font-bold">OpenRouter AI Integration</h1>
<p>WebVM is integrated with OpenRouter, providing access to Claude and other models through a unified API.</p>
<p>You need to provide your OpenRouter API key. The key is only saved locally to your browser.</p>
<p class="text-sm text-gray-300 mt-1">Get free credits or purchase at <a href="https://openrouter.ai" target="_blank" class="underline">openrouter.ai</a></p>
...
{#if $apiState == "KEY_REQUIRED"}
	<textarea class="bg-neutral-700 p-2 rounded-md placeholder-gray-400 resize-none shrink-0" placeholder="Insert your OpenRouter API Key (sk-or-v1-...)" rows="1" on:keydown={handleKeyEnter} on:input={handleResize} id="ai-input"/>
{:else if $aiActivity}
	{#if stopRequested }
