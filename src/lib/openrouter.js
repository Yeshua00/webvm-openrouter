...
// Use Claude Sonnet 4 through OpenRouter - supports computer use tools
...
// Store for selected model (allows user to change if needed)
export const selectedModel = writable(DEFAULT_MODEL);
...
async function sendMessages(handleTool)
{
	...
	try
	{
		...
		// Make request to OpenRouter's Anthropic-compatible endpoint
		const response = await fetch(`${OPENROUTER_API_BASE}/messages`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
				'Authorization': `Bearer ${apiKey}`,
				// OpenRouter-specific headers
				'HTTP-Referer': typeof window !== 'undefined' ? window.location.origin : 'https://webvm.io',
				'X-Title': 'WebVM OpenRouter'
			},
		...
		for(var i=0;i<messages.length;i++)
		{
			...
			if(Array.isArray(c))
			{
				if(c[0] && c[0].type == "tool_result" && c[0].content && c[0].content[0] && c[0].content[0].type == "image")
					delete c[0].content;
			}
		}
		...
		for(var i=0;i<content.length;i++)
		{
			...
			if(c.type == "text")
			{
				addMessageInternal("assistant", c.text);
			}
			else if(c.type == "tool_use")
			{
				addMessageInternal("assistant", [c]);
				...
				if(stopFlag)
				{
					// Maintain state consistency by stopping after adding a valid response
					aiActivity.set(false);
					return;
				}
		...
		{
			tryPlausible("OpenRouter Success");
			aiActivity.set(false);
		}
	}
	catch(e)
	{
		tryPlausible("OpenRouter Error");
		if(e.status == 401)
		{
		...
		{
			addMessageInternal('error', 'API key rejected. Make sure you have credits in OpenRouter.');
		}
		else
		{
			addMessageInternal('error', e.error?.message || String(e));
		}
...
export function getMessageDetails(msg) {
	const isToolUse = Array.isArray(msg.content) && msg.content[0] && msg.content[0].type === "tool_use";
	const isToolResult = Array.isArray(msg.content) && msg.content[0] && msg.content[0].type === "tool_result";
	const isThinking = Array.isArray(msg.content) && msg.content[0] && msg.content[0].type === "thinking";
	let icon = "";
	let messageContent = "";
...
{
	// Find the CheerpX textarea, it's attached to the body element
...
function initialize()
{
	var savedApiKey = localStorage.getItem("openrouter-api-key");
	if(savedApiKey)
